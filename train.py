...
def MILL(element_logits, seq_len, batch_size, labels, device):
    ...
    for i in range(batch_size):
        tmp, _ = torch.topk(element_logits[i][:max(seq_len[i], 1)], k=max(int(k[i]), 1), dim=0)
...
def CASL(x, element_logits, seq_len, n_similar, labels, device):
    ...
    for i in range(0, n_similar * 2, 2):
        ...
        sim_loss = sim_loss + 0.5 * torch.sum(
            torch.max(d1 - d2 + 0.5, torch.FloatTensor([0.]).to(device)) * Variable(labels[i, :]) * Variable(
                labels[i + 1, :]))
        sim_loss = sim_loss + 0.5 * torch.sum(
            torch.max(d1 - d3 + 0.5, torch.FloatTensor([0.]).to(device)) * Variable(labels[i, :]) * Variable(
                labels[i + 1, :]))
